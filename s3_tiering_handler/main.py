...
def lambda_handler(event, context):
    ...
    for docs in to_standard_ia:
        s3_file_name = docs["s3_file_path"]
        s3_service.move_file(docs["s3_bucket_name"], s3_file_name, S3_BUCKET_STANDARD_IA, s3_file_name, "STANDARD_IA")
        dynamodb_service.update_ddb_record(docs['PK'], docs["SK"], storage_class="STANDARD_IA", s3_bucket_name=S3_BUCKET_STANDARD_IA)
